...
class PrendasCombinacion(models.Model):
    prenda = models.ForeignKey(Prenda, on_delete=models.CASCADE, related_name='combinaciones')
    combinacion = models.ForeignKey(Combinacion, on_delete=models.CASCADE, related_name='prendas')

    class Meta:
        unique_together = ('prenda', 'combinacion')

    def __str__(self):
        return f"{self.prenda.nombre} en {self.combinacion.categoria}"
